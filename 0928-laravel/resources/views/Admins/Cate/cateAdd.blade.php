...
<div class="formbody">
    <div id="usual1" class="usual">
        <div id="tab1" class="tabson">
            <form action="{{url('Admins/cateAddAction')}}" method="post" id="form1" onsubmit="fun()" class="layui-form">
            <ul class="forminfo">
                {{csrf_field()}}
                <li>
                    <label>添加分类<b>*</b></label>
                    <input name="name" type="text" class="dfinput" placeholder="请填写分类名称"  style="width:518px;"/>
                </li>
                <li>
                    ...
                    <div class="vocation">
                        <select name="id">
                            ...
                            @foreach($rows as $v)
                            <option value="{{$v->id}}"> |-{{str_repeat("----",substr_count($v->path,','))}}{{$v->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </li>
