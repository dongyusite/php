...
<div class="formbody">
    <div id="usual1" class="usual">
        <div id="tab2" class="tabson">
            <!-- 搜索 -->
            <form action="{{url('Goods/goodsList')}}" method="post">
                <ul class="seachform">
...
{{--                        <td>{{$v->goods_pic}}</td>--}}
                        ...
                        <td>
                            <a href="{{url('Goods/goodsEdit')}}?id={{$v->gid}}" class="tablelink">编辑</a>
...
<script>
    function goodsDel(id) {
        var bool=confirm('确定要删除嘛?');
        if(bool){
            var _token='{{csrf_token()}}';
            var  gid='id_'+id;
            $("#"+gid).remove();
            $.post("{{url('Goods/ajaxgoodsDel')}}",{id:id,_token:_token},function (data) {
                if(data.status ===1){
                 alert('删除成功!')
                }
            },'json')
