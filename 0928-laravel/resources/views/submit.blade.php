...
<div class="beij_center">
	...
	<div class="checkout_steps">
		...
		@foreach($form as $v)
        <div class="step-tit" id="ids_{{$v->id}}">
            <h3>送货清单</h3> 
            <div class="extra_r">
            	<a href="#" class="ftx-05 J_consignee_global">价格说明</a>
            	<a href="javascript:Delform({{$v->id}})" class="ftx-05 J_consignee_global">删除订单</a>
            </div>
        </div>
        <div class="shopping_list" id="id_{{$v->id}}" >
        	<div class="dis_modes">
        	...
        	<div class="goods_list" >
        		...
        		<div class="goods_list_neik">
        			<h4 class="vendor_name_h">商家：{{$v->g_shop}}服装旗舰店</h4>
        			<div class="goods_item">
        				<div class="p_img"><a href="{{url('Page/goodsDetail',[$v->gid])}}"><img src="{{asset("/upload/$v->g_pic")}}" style="width: 100px;height: 80px"></a></div>
        				<div class="goods_msg">
        					<div class="p_name">
        						<a href="{{url('Page/goodsDetail',[$v->gid])}}">{{$v->g_name}}</a>
        					</div>
        					<div class="p_price">
        						<span class="jq_price">￥{{$v->g_total}}</span>
        						<span>{{$v->g_num}}</span>
        						<span>有货</span>
        						<span>1.170kg</span>
        					</div>
		...
		@endforeach
		<div class="pagin">
			<div class="message">共<i class="blue">{{$form->total()}}</i>条记录，当前显示第&nbsp;<i class="blue">{{$form->currentPage()}}
					&nbsp;</i>页</div>
			{{$form->links()}}
	...
	<div class="order_summary">
		<div class="qianq_mx">
			<div class="jiaq_meih">
				<span class="xiangq_leib"><em class="goumai_ges">{{$count}}</em> 件商品，总商品金额：</span>
				<em class="goum_zongj">￥{{$total}}</em>
			</div>
			<div class="jiaq_meih">
		...
		<div class="dsgs">
			<div class="qianq_mx">
				<div class="jiaq_meih">
					...
					<em class="goum_zongj zhongf_jine">￥{{$total}}</em>
				</div> 
			</div>
		</div>
		<div class="zuiz_diz">
...
<script>
	function Delform(id) {
		if(confirm('我想吃烤山药?')){
			$('#id_'+id).remove();
			$('#ids_'+id).remove();
			$.get("{{url('Page/Del_form')}}",{id:id},function (data) {
				if(data.status === 0){
					alert('删除成功!');
				}
			},'json')
